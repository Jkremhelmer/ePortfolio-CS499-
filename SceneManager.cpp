...
void SceneManager::DestroyGLTextures()
{
	for (int i = 0; i < m_loadedTextures; i++)
	{
		glDeleteTextures(1, &m_textureIDs[i].ID); //updated to glDeleteTextures to free up GPU memory associated with them
	}
}
...
bool SceneManager::FindMaterial(std::string tag, OBJECT_MATERIAL& material)
{
	...
	return(bFound); //code always returns true in this instance. Going to update to correctly indicate whether the material was found
}
...
/*** for assistance.                                        ***/
/**************************************************************/
//Used to set textures for the scene
void SceneManager::LoadSceneTextures()
{
	...
	bReturn = CreateGLTexture(
		"../../Utilities/textures/Rocks.jpg",
		"coaster"); //Romeved duplicate texture

	BindGLTextures();
}
// This section sets the material properties for the scene
void SceneManager::DefineObjectMaterials()
{
	...
	coasterMaterial.ambientColor = glm::vec3(0.1f, 0.1f, 0.1f);
	coasterMaterial.ambientStrength = 0.2f;
	coasterMaterial.diffuseColor = glm::vec3(0.5f, 0.5f, 0.5f);
	coasterMaterial.specularColor = glm::vec3(0.1f, 0.1f, 0.1f);
	coasterMaterial.shininess = 0.3;
	coasterMaterial.tag = "coaster";

	m_objectMaterials.push_back(coasterMaterial);

	OBJECT_MATERIAL alexaMaterial;
	alexaMaterial.ambientColor = glm::vec3(0.1f, 0.1f, 0.1f);
	alexaMaterial.ambientStrength = 0.1f;
	alexaMaterial.diffuseColor = glm::vec3(0.3f, 0.2f, 0.3f);
	alexaMaterial.specularColor = glm::vec3(0.4f, 0.2f, 0.2f);
	alexaMaterial.shininess = 0.5;
	alexaMaterial.tag = "alexa";

	m_objectMaterials.push_back(alexaMaterial);
}

//This section sets the lighting for the scene
 ...
 *  RenderMartiniGlass()
 *
 *  This method is called to render the shapes for the Martini
 *  glass object.
 ***********************************************************/
	void SceneManager::RenderMartiniGlass()
	{
		// declare the variables for the transformations
		glm::vec3 scaleXYZ;
		float XrotationDegrees = 0.0f;
		...
		float ZrotationDegrees = 0.0f;
		glm::vec3 positionXYZ;
	// Cylinder for bottom of glass
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(1.0f, .05f, 0.5f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.0f, 0.55f, 6.0f);
	...
	SetShaderColor(.047, .164, .960, 1.0);
	...
	//Cylinder for stwm of Glass
	// set the XYZ scale for the mesh
	scaleXYZ = glm::vec3(.05f, 3.0f, .05f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.0f, .6f, 6.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.0f, 4.55f, 6.0f);
...
*  RenderAlexaDOT()
*
*  This method is called to render the shapes for the Alexa DOT object.
***********************************************************/
	void SceneManager::RenderAlexaDOT()
	{
		// declare the variables for the transformations
		glm::vec3 scaleXYZ;
		float XrotationDegrees = 0.0f;
		...
		// set the XYZ position for the mesh
		positionXYZ = glm::vec3(-0.5f, 4.0f, 3.0f);
		...
		SetShaderColor(.938, .938, .897, 1.0);
		SetShaderMaterial("alexa");
...
*  RenderCoaster()
*
*  This method is called to render the shapes for the Coaster object.
***********************************************************/
	...
	void SceneManager::RenderCoaster()
	{
		...
		// set the XYZ scale for the mesh
		scaleXYZ = glm::vec3(2.0f, 0.5f, 1.0f);
...
*  This method is called to render the shapes for the Alexa DOT object.
***********************************************************/
	void SceneManager::RenderDocker()
	{
		// declare the variables for the transformations
		glm::vec3 scaleXYZ;
		float XrotationDegrees = 0.0f;
		...
		// set the XYZ position for the mesh
		positionXYZ = glm::vec3(-0.8f, 2.8f, 3.0f);
